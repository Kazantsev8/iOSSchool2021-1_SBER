...
//MARK: KEYWORDS
//MARK: FINAL
//Данное ключевое слово позволяет выполнить static method dispatch для метод, определенного в классе (как для value type). Оно исключает возможность какого-либо динамического поведения, а так же скрывает метод от Objective-C runtime. В данном случае селектор не создается. Так же оно запрещает наследование от класса. Использование этого ключевого слова позволяет компилятору сделать некоторые оптимизации для повышения производительности. Применяется только к классам и его методам.
//MARK: DYNAMIC
//Ключевое слово dynamic позволяет методу вызываться с помощью message dispatch. Данное слово неявно помечает метод как @objc и делает его видимым для Objective-C runtime. В Swift не указано, что dynamic доступен только для классов, однако Struct и Enum не поддерживают наследование. Таким образом во время выполнения структур и перечислений динамическая диспетчеризация не работает.
//MARK: @OBJC
//Ключевое слово @objc не изменяет способ отправки метода. Оно просто делает метод видимым для среды выполнения Objective-C. Чаще всего @objc используется для создания селекторов, а так же позволяет переопределять методы, объявленные в расширениях класса (по умолчанию это нельзя делать из-за static dispatch).
//MARK: @NONOBJC
//Ключевое слово @nonobjc изменяет отправку метода. Оно может быть использовано для отключения message dispatch и сделать метод невидимым для Objective-C runtime. Кажется, что нету разницы между @nonobjc и final, поэтому использование final делает код более понятным.
//MARK: @OBJC FINAL
//Ключевое слово @objc final включает статическую отправку метода и регестрирует селектор метода во врем выполнения Objective-C. Ключевое слово позволяет методу реагировать при выполнении селектора или других функций, связанных с Objective-C,А так же обеспечивает производительность статической диспетчеризации.
class SomeClass {
    //direct dispatch, visible for Objective-C runtime
    @objc final func move() {}
}
//Если свойство наблюдается с помощью KVO и это свойство улучшено до Direct Dispatch, код все равно будет компилироваться, но динамический сгенерированный метод KVO не будет запущен.
